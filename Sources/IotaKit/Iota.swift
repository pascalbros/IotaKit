...
public class Iota: IotaDebuggable {
	...
	/// These trytes can then be easily converted into the actual transaction object. See IotaTransaction.
	///
	/// - Parameters:
	...
	/// returns the addresses, transfers, inputs and balance that are associated
	/// and have been used with your account (seed). This function is useful in getting all the relevant information of your account.
	///
	/// - Parameters:
	///   - seed: The Seed.
	///   - minimumNumberOfAddresses: The minimum number of addresses associated to the seed, setting it to `true` will
	///   be useful in order to speed up the process, by requesting all the addresses balance and transactions in batch.
	///   - security: The security level (1-3).
	///   - requestTransactions: Setting it to `true`, all the transactions associated to the addresses
	///   will be requested and returned. If you don't need it, set it to `false`.
	///   - success: Success block.
	///   - error: Error block.
	///   - log: Log block.
	...
	public func accountData(
		seed: String,
		minimumNumberOfAddresses: Int = 0,
		security: Int = 2,
		requestTransactions: Bool = false,
		_ success: @escaping (_ account: IotaAccount) -> Void,
		error: @escaping (Error) -> Void, log: ((_ log: IotaLog) -> Void)? = nil) {
		...
		func findBalances(_ requestTxs: Bool) {
			...
			if requestTxs {
				...
				self.balances(addresses: addresses, { (balances) in
					...
					account.balance = balances.reduce(into: Int64(0)) { $0 += $1.value }
					success(account)
				}, error: { err in
					error(err)
				})
	...
	public func attachToTangle(
		...
		index: Int,
		security: Int = 2,
		_ success: @escaping (_ transfer: IotaTransaction) -> Void,
		error: @escaping (Error) -> Void) {
		let address = IotaAPIUtils.newAddress(seed: seed, security: security, index: index, checksum: false, curl: self.curl.clone())
	...
	///   - reference: Hash of transaction to start random-walk from, used to make sure
	///   the tips returned reference a given transaction in their past, leave it as default.
	///   - success: Success block.
	///   - error: Error block.
	public func sendTransfers(
		...
		transfers: [IotaTransfer], inputs: [IotaInput]?,
		remainderAddress: String?, reference: String? = nil,
		_ success: @escaping (_ transactions: [IotaTransaction]) -> Void,
		error: @escaping (Error) -> Void) {
		self.prepareTransfers(seed: seed,
							  security: security,
							  transfers: transfers,
							  remainder: remainderAddress,
							  inputs: inputs,
							  validateInputs: false, { trytes in
	...
	/// Takes a tail transaction hash as input, gets the bundle associated with the transaction and then
	/// replays the bundle by attaching it to the tangle.
	///
	/// - Parameters:
		...
		func continueWithBundle(bundle: IotaBundle) {
			...
			for i in 0..<bundle.transactions.count {
				...
				if bundleValue < 0 {
					...
					for j in i+1..<bundle.transactions.count {
						let newBundleTx = bundle.transactions[j]
						if newBundleTx.address == address &&
							newBundleTx.value == 0 &&
							signature.signatureFragments.firstIndex(of: newBundleTx.signatureFragments) == nil {
			...
			for aSignaturesToValidate in signaturesToValidate {
				...
				let isValidSignature =
					IotaSigning(curl: self.curl.clone()).validateSignature(expectedAddress: address, signatureFragments: signatureFragments, bundleHash: bundleHash)
				if !isValidSignature {
	...
	///   - success: Success block.
	///   - error: Error block.
	public func transactionsFromAddress(
		address: String,
		_ success: @escaping (_ transactions: [IotaTransaction]) -> Void,
		error: @escaping (Error) -> Void) {
	...
	///   - success: Success block.
	///   - error: Error block.
	public func tailFromTransaction(
		tx: IotaTransaction,
		_ success: @escaping (_ tail: IotaTransaction) -> Void,
		error: @escaping (Error) -> Void) {
	...
	///   - success: Success block.
	///   - error: Error block.
	public func promoteTransaction(
		hash: String,
		transactions: [IotaTransfer] = [Iota.spamTransfer],
		depth: Int = 10, minWeightMagnitude: Int = IotaConstants.mwm,
		delayInSeconds: UInt = 0, numberOfPromotes: Int = 4,
		_ success: @escaping (_ tail: String) -> Void,
		error: @escaping (Error) -> Void) {
	...
	///   - error: Error block.
	public func promoteTransaction(
		_ tx: IotaTransaction,
		transactions: [IotaTransfer] = [Iota.spamTransfer],
		depth: Int = 10, minWeightMagnitude: Int = IotaConstants.mwm,
		delayInSeconds: UInt = 0, numberOfPromotes: Int = 4,
		_ success: @escaping (_ tail: String) -> Void, error: @escaping (Error) -> Void) {
	...
	public func promote(
		...
		self.isPromotable(tail: tail, { (result) in
			if result {
				self.internalPromote(tail: tail, numberOfPromotes: numberOfPromotes, success, error: error)
			} else {
	...
	///   - depth: Number of bundles to go back to determine the transactions for approval, leave it as default.
	///   - minWeightMagnitude: Minimum weight magnitude, leave it as default.
	///   - reference: Hash of transaction to start random-walk from, used to make sure the tips returned
	///   reference a given transaction in their past, leave it as default.
	///   - success: Success block.
	///   - error: Error block.
	public func sendTrytes(
		trytes: [String],
		depth: Int = 10,
		minWeightMagnitude: Int = IotaConstants.mwm,
		...
		//4
		func toTxs(trytes: [String]) {
			IotaDebug("Converting to transactions")
			let trx = trytes.map { IotaTransaction(trytes: $0) }
			success(trx)
		}

		//3
		func store(trytes: [String]) {
			IotaDebug("Storing trytes")
			APIServices.storeTransactions(nodeAddress: self.address, trytes: trytes, {
				toTxs(trytes: trytes)
			}, error)
		}

		//2
		func broadcast(trytes: [String]) {
			IotaDebug("Broadcasting trytes")
			APIServices.broadcastTransactions(nodeAddress: self.address, trytes: trytes, {
				store(trytes: trytes)
			}, error)
		}
		...
		func attach(trunkTx: String, branchTx: String) {
			...
			self.attachToTangle(trunkTx: trunkTx, branchTx: branchTx, minWeightMagnitude: minWeightMagnitude, trytes: trytes, { ttt in
				broadcast(trytes: ttt)
			}, error: error)
		}
...
extension Iota {
	...
	internal func internalPromote(
		tail: String,
		transactions: [IotaTransfer] = [Iota.spamTransfer],
		depth: Int = 10,
		minWeightMagnitude: Int = IotaConstants.mwm,
		delayInSeconds: UInt = 0, index: Int = 0,
		numberOfPromotes: Int,
		_ success: @escaping (_ tail: String) -> Void,
		error: @escaping (Error) -> Void) {
		...
		self.sendTransfers(seed: Iota.spamSeed,
						   security: 2,
						   depth: depth,
						   minWeightMagnitude: minWeightMagnitude,
						   transfers: [Iota.spamTransfer],
						   inputs: nil,
						   remainderAddress: nil,
						   reference: tail, { tx in
			self.IotaDebug("Promoted, tx:\(tx.first?.hash ?? "")")
			self.internalPromote(tail: tail, index: index + 1, numberOfPromotes: numberOfPromotes, success, error: error)
		}, error: error)
	}

	internal func traverseBundle(
		trunkTx: String,
		bundleHash: String?,
		bundle: IotaBundle,
		_ success: @escaping (_ transfer: IotaBundle) -> Void,
		error: @escaping (Error) -> Void) {
		...
		self.trytes(hashes: [trunkTx], { (txs) in
			...
			guard !trx.trytes.isEmpty else { error(IotaAPIError("Invalid bundle")); return }
			guard !trx.bundle.isEmpty else { error(IotaAPIError("Invalid bundle")); return }

			if bundleHash == nil {
				bundleHash = trx.bundle
			}
			if bundleHash! != trx.bundle {
				success(bundle)
				return
			}
			...
			DispatchQueue.global(qos: .userInitiated).async {
				self.traverseBundle(trunkTx: trunkTx, bundleHash: bundleHash, bundle: theBundle, success, error: error)
			}
		}, error: error)
	}

	internal func attachToTangle(
		seed: String,
		address: String,
		security: Int = 2,
		_ success: @escaping (_ transfer: IotaTransaction) -> Void,
	...
	internal func prepareTransfers(
		seed: String, security: Int,
		transfers: [IotaTransfer],
		remainder: String?,
		inputs: [IotaInput]?,
		validateInputs: Bool,
		_ success: @escaping (_ transfers: [String]) -> Void,
		error: @escaping (Error) -> Void) {
		...
		if totalValue != 0 {
			if let inputs = inputs, inputs.isEmpty {
				...
				self.inputs(seed: seed, security: security, threshold: totalValue, canDoubleSpend: false, { (resultInputs) in
					...
					if remainder == nil {
						remainderAddress =
							IotaAPIUtils.newAddress(seed: seed, security: security, index: resultInputs.accountAddresses.count, checksum: false, curl: self.curl.clone())
					}
					self.addRemainder(
						seed: seed,
						security: security,
						inputs: resultInputs.inputs,
						bundle: bundle,
						tag: tag,
						totalValue: Int64(totalValue),
	...
	internal func inputs(
		seed: String,
		security: Int,
		threshold: UInt64,
		canDoubleSpend: Bool = false,
		_ success: @escaping (_ inputs: InputsAndAddresses) -> Void,
		error: @escaping (Error) -> Void) {
		...
		self.accountData(seed: seed, security: security, requestTransactions: true, { (account) in
			...
			for i in 0..<filteredAddresses.addresses.count {
				let value = filteredAddresses.addresses[i]
				let input = IotaInput(address: value.hash, balance: value.balance!, keyIndex: value.index!, security: security)
				inputs.append(input)
			}
	...
	internal func filterAddresses(
		_ addresses: [IotaAddress],
		amountToReach: UInt64,
		canDoubleSpend: Bool = false) -> (addresses: [IotaAddress], balance: UInt)? {

		let filteredAddresses = addresses.filter { value -> Bool in
			let balance = value.balance! > 0
			let doubleSpend = canDoubleSpend ? true : value.canSpend!
			return balance && doubleSpend
		}

		var currentValue: UInt = 0
		var resultAddresses: [IotaAddress] = []
		for value in filteredAddresses {
			currentValue += UInt(value.balance!)
			resultAddresses.append(value)
	...
	public func attachToTangle(
		...
		if let localPow = self.localPoW {
			...
			DispatchQueue.global(qos: .userInitiated).async {
				for tx in trytes {
					var txn = IotaTransaction(trytes: tx)
			...
			APIServices.attachToTangle(nodeAddress: self.address,
									   trunkTx: trunkTx,
									   branchTx: branchTx,
									   minWeightMagnitude: minWeightMagnitude,
									   trytes: trytes, { resultTrytes in
				success(resultTrytes)
			}, { err in
				error(err)
			})
	...
	internal func newAddress(
		...
		total: Int,
		returnAll: Bool,
		_ success: @escaping (_ addresses: [String]) -> Void,
		error: @escaping (Error) -> Void) {
		...
		self.accountData(seed: seed, security: security, { (account) in
			let newAddress =
				IotaAPIUtils.newAddress(seed: seed, security: security, index: account.addresses.count, checksum: checksum, curl: self.curl.clone())
	...
	internal func addRemainder(
		...
		let inputsTotal = inputs.reduce(into: Int64(0)) { $0 += $1.balance }
