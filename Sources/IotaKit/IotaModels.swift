...
public struct IotaBundle {
	internal static let emptyHash = String(repeating: "9", count: 81)
...
public struct IotaAddress {
	...
	/// The current balance of the address, if `nil`, it can be derived from the list of transactions.
	/// In that case there is no guarantee that the value is updated.
	public var balance: Int64? {
		get { _balance ?? calculatedBalance }
		set(newValue) { self._balance = newValue }
	}

	private var _canSpend: Bool?

	/// Determines if an addres can be spent or not.
	public var canSpend: Bool? {
		get { _canSpend ?? calculateCanSpend }
		set(newValue) { self._canSpend = newValue }
	}
	...
	private var calculatedBalance: Int64? {
		...
		return txs.reduce(0) { (res, trx) -> Int64 in
			let value = trx.persistence ? trx.value : 0
			return res+value
		}
	}
	...
	private var calculateCanSpend: Bool? {
		guard let txs = self.transactions else { return nil }
		if txs.first(where: { $0.value < 0 }) != nil { return false }
		return true
	}
}
